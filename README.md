...
- **반복적인 로그 조회 작업**의 비효율성
- **CS 처리 수작업 부담**

### 솔루션: Agentic Workflow

**Agentic Workflow**는 LLM의 추론 능력을 활용하되, Workflow와 Tool로 행동 범위를 통제하여 예측 가능한 결과를 보장하는 설계 패턴입니다.

#### 핵심 아이디어

- **LLM 역할 제한**: 문의 분류에만 사용하여 확률적 불확실성 최소화
- **코드 기반 처리**: 분류 후 실제 작업은 결정론적 코드로 수행
- **구조화된 워크플로우**: 각 단계의 입출력을 DTO로 명확히 정의

#### 자동화 범위

| 영역 | 처리 방식 |
|------|----------|
| 간단한 작업 | 로그 조회 및 자동 응답 생성 |
| 권한 필요 작업 | 개발자 승인 후 수행 |
| 사전 정의 케이스 | 분류 규칙 기반 자동 처리 |
| 미정의 케이스 | 정보 요청 또는 에스컬레이션 |

## 🛠 기술 스택

### Core Framework

- **[Microsoft Agent Framework](https://learn.microsoft.com/en-us/agent-framework/)**: C# 기반 에이전트 프레임워크 (2025년 12월 기준
  프리뷰)
...
시스템은 3개의 계층으로 구성되어 책임을 명확히 분리합니다:

### 계층별 책임

| 계층           | 특징                | 책임                                                  |
|--------------|-------------------|-----------------------------------------------------|
| **Agent**    | LLM 기반 추론 및 의사결정  | • 문의 분류<br>• 자연어 이해<br>• 컨텍스트 기반 판단                 |
| **Executor** | Workflow 오케스트레이션  | • Agent와 Service 연결<br>• 데이터 변환 (DTO)<br>• 작업 흐름 제어 |
| **Service**  | 비즈니스 로직 및 데이터 액세스 | • DB 조회/갱신<br>• 파일 I/O<br>• 재사용 가능한 로직              |

### 워크플로우 실행 흐름

4개의 노드(Executor)로 구성된 선형 Workflow
```
